...
import com.thinkgem.jeesite.modules.sys.dao.TagDao;
...
public class SystemService extends BaseService implements InitializingBean {
	...
	public List<User> findUserByRole(String roleId){
		...
		for(User user : list){
			if(userDao.findRoleidListByUserid(user.getId() , roleId) != 0){
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	 ...
	 */
	@Transactional(readOnly = false)
	public boolean deleteBroker(List<DyClient>list,User user) throws ServiceException{
		boolean isDelete = false;
		//删除经纪人
		this.deleteUser(user);
		//重新分配经纪人
		for(int i = 0;i<list.size();i++){
			DyClient dyClient = list.get(i);
			// 自动分配经纪人
			User broker =dyClientService.getRandomBroker();
			dyClient.setBrokerId(broker.getId());
			int n = dyClientDao.update(dyClient);
			if(n==0){
				throw new ServiceException("删除经纪人时，重新分配经济人出错");
			}
		}
		isDelete = true;
		return isDelete;
