...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			//$("#name").focus();
			if("${dyDomainname.id}" != null && typeof("${dyDomainname.id}")!="undefined" && "${dyDomainname.id}" != ""){
				$("#nameId").attr({readonly:'true'});
				$("#dyidId").attr({readonly:'true'});
				$("#bonusShareTotalId").attr({readonly:'true'});
				$("#bonusShareNumberId").attr({readonly:'true'});
			}
			$("#inputForm").validate({
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/sys/dy/dyDomainname/form?id=${dyDomainname.id}">域名信息<shiro:hasPermission name="sys:dy:dyDomainname:edit">${not empty dyDomainname.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:dy:dyDomainname:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="dyDomainname" action="${ctx}/sys/dy/dyDomainname/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input id="nameId" path="name" htmlEscape="false" maxlength="63" class="input-xlarge required"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="dyClient.dyid" id="dyidId" htmlEscape="false" maxlength="64" class="input-xlarge required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<c:set var="patten" value="yyyy-MM-dd HH:mm:ss"></c:set>
				<c:if test="${dyDomainname.status == '00' || dyDomainname.status == '01' || dyDomainname.status == '02'}">
					<c:set var="patten" value="yyyy-MM-dd"></c:set>
				</c:if>
				<input name="endTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${dyDomainname.endTime}" pattern="${patten }"/>"
					onclick="WdatePicker({dateFmt:'${patten }',isShowClear:false});"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
		...
		<c:if test="${not empty dyDomainname.id}">
			<div class="control-group">
				...
				<div class="controls">	
						<form:hidden  path="status" htmlEscape="false" class="input-xlarge  digits"/>
		...
		<!-- 		<input type="button" value="压缩" class="btn"  onclick="setImage('nameImage2')" > -->
		...
		<div class="form-actions">
			<shiro:hasPermission name="sys:dy:dyDomainname:edit">
				<input id="btnSubmit" class="btn btn-primary" type="submit" onclick="btnSubmitFunction()" value="保 存"/>&nbsp;
			</shiro:hasPermission>
