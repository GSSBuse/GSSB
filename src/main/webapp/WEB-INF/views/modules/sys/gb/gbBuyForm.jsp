...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/sys/gb/gbBuy/">买标信息管理列表</a></li>
		<li class="active"><a href="${ctx}/sys/gb/gbBuy/form?id=${gbBuy.id}">买标信息管理<shiro:hasPermission name="sys:gb:gbBuy:edit">${not empty gbBuy.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:gb:gbBuy:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="gbBuy" action="${ctx}/sys/gb/gbBuy/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>		
		<div class="control-group">
			<label class="control-label">用户ID：</label>
			<div class="controls">
				<form:input path="gbclientId" htmlEscape="false" maxlength="64" class="input-xlarge "/>
		...
		<div class="control-group">
			...
			<select name="status">
				 	<option value="0" <c:if test="${'0' eq gbBuy.status}">selected="selected"</c:if>>交易结束</option>
				 	<option value="1" <c:if test="${'1' eq gbBuy.status}">selected="selected"</c:if>>正在交易</option>
			</select>
			<span class="help-inline"><font color="red">*</font> </span>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<input name="createTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${gbBuy.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="dealTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${gbBuy.dealTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div>
		<div class="form-actions">
			<shiro:hasPermission name="sys:gb:gbBuy:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
