...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
							...
							/*充值提现通知消息数量*/
								$.get("${ctx}/sys/dy/dyCashFlow/cUnconfirmCount?_="+(new Date()).getTime(),function(data3){
									...
									if(cUnconfirmCount>0){
										...
										$("#cashUnconfirmCount").hide();
									}
								/*计算所有的消息数量*/
								var total = unmarkCount + unconfirmCount + unfinishCount + cUnconfirmCount;
								if (total > 0){
									$("#notifyNum").show().html("("+total+")");
								}else{
									$("notifyNum").hide()
								}
							});
			...
			function getNotifyNumForSecurity(){
				/*会员认证通知消息数量*/
				var unmarkCount = 0;
				var unconfirmCount= 0;
				var unfinishCount=0;
				$.get("${ctx}/sys/dy/dyClient/unmarkCount?_="+(new Date()).getTime(),function(data){
					...
					$.get("${ctx}/sys/dy/dyDomainname/unconfirmCount?_="+(new Date()).getTime(),function(data1){
						...
						$.get("${ctx}/sys/dy/dyDomainname/unfinishCount?_="+(new Date()).getTime(),function(data2){
							...
							$("#cashUnconfirmCountId").hide();
								/*计算所有的消息数量*/
								var total = unmarkCount + unconfirmCount + unfinishCount;
								if (total > 0){
									$("#notifyNum").show().html("("+total+")");
								}else{
									$("notifyNum").hide()
			...
			function getNotifyNumForMoney(){
				...
				$.get("${ctx}/sys/dy/dyCashFlow/cUnconfirmCount?_="+(new Date()).getTime(),function(data3){
					...
					if(cUnconfirmCount>0){
						$("#cashUnconfirmCount").show().html("("+cUnconfirmCount+")");
						$("#notifyNum").show().html("("+cUnconfirmCount+")");
					}else{
						$("#cashUnconfirmCount").hide();
						$("notifyNum").hide()
					}
					$("#notifyNum2Id").hide();
					$("#domainUnconfirmCountId").hide();
					$("#domainUnfinishCountId").hide();
				});
			}
			function getNotifyNum (){
				...
				if(temp == "isMoney"){
					/*财务调用*/
					getNotifyNumForMoney();
				}else if(temp == "isAdmin"){
					/*管理员调用*/
					getNotifyNumForAdmin();
				}else if(temp == "isSecurity"){
					/*经纪人调用*/
					getNotifyNumForSecurity();
				}
			}
			getNotifyNum();
				...
				<ul id="userControl" class="nav pull-right">
					...
					<li id="userInfo" class="dropdown">
						...
						<ul class="dropdown-menu">
							...
							<li id="notifyNum2Id"><a href="${ctx}/sys/dy/dyClient?authenticationMark=2" target="mainFrame"><i class="icon-bell"></i>&nbsp;  会员认证通知 <span id="notifyNum2" class="label label-info hide"></span></a></li>
						    <li id="domainUnconfirmCountId"><a href="${ctx}/sys/dy/dyDomainname?status=01" target="mainFrame"><i class="icon-bell"></i>&nbsp;  域名审核通知<span id="domainUnconfirmCount" class="label label-info hide"></span></a></li>
							<li id="domainUnfinishCountId"><a href="${ctx}/sys/dy/dyDomainname/deal?status=14" target="mainFrame"><i class="icon-bell"></i>&nbsp;  域名成交通知<span id="domainUnfinishCount" class="label label-info hide"></span></a></li>
							<li id="cashUnconfirmCountId"><a href="${ctx}/sys/dy/dyCashFlow?type=2" target="mainFrame"><i class="icon-bell"></i>&nbsp;  充值提现通知<span id="cashUnconfirmCount" class="label label-info hide"></span></a></li>
						</ul>
					</li>
