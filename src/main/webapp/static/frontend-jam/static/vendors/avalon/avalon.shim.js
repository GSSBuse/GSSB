...
function addEventListener(elem, type, fn, useCap) {
	if (window.$) {
		$(elem).bind(type, fn);
	} else {
		if (elem.addEventListener) {
            elem.addEventListener(type, fn, useCap)
        } else {
...
function removeEventListener(elem, type, fn, useCap) {
	if (window.$) {
		$(elem).unbind(type, fn);
	} else {
		if (elem.removeEventListener) {
            elem.removeEventListener(type, fn, useCap)
        } else {
...
//        }
        addEventListener(el, type, callback, !!phase)
        return callback
    },
    /*卸载事件*/
...
//        }
        removeEventListener(el, type, callback, !!phase)
    },
    /*读写删除元素节点的样式*/
...
//            }
        	addEventListener(elem, type, callback, false)
